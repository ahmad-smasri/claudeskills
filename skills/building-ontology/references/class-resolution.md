...
python3 - <<'PY'
import openpyxl
wb = openpyxl.load_workbook('reference-models/QF_SSC_Ontology_V04.xlsx', data_only=True)
...
`QF_SSC_Ontology_V04.xlsx` (the QF SSC building) is the first to check - it is
...
its misspelled tab `HQ_Onotlogy`. Later projects go here too as they
land.
