...
| `scripts/build_review_workbook.py` | runs both checkers plus seven checks neither covers, then writes the house review workbook - a `Claude Log`, a `Review_Summary`, the ontology sheet with flagged rows filled yellow/amber, and one issue sheet per entity family | a delivered sheet needs reviewing, or a client asks what is wrong with a draft |
